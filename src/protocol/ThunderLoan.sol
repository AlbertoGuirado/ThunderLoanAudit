...
// e should implement this
//import { IThunderLoar } from "../interfaces/IFlashLoanReceiver.sol";
...
contract ThunderLoan is Initializable, OwnableUpgradeable, UUPSUpgradeable, OracleUpgradeable {
    ...
    /*//////////////////////////////////////////////////////////////
                            STATE VARIABLES
    //////////////////////////////////////////////////////////////*/
    mapping(IERC20 => AssetToken) public s_tokenToAssetToken; // e ? Its USDC -> USDCAssetToken

    // The fee in WEI, it should have 18 decimals. Each flash loan takes a flat fee of the token price.
    // @ audit-info it should be constant or inmutable
    uint256 private s_feePrecision; // @ audit-info constant
    uint256 private s_flashLoanFee; // 0.3% ETH fee

    mapping(IERC20 token => bool currentlyFlashLoaning) private s_currentlyFlashLoaning; // e Tells us if the token is in the MIDDLE OF A FLASHLOAN
    ...
    // what happen if we deploy the contract and someone else initializes it?
    // ops
    // they could pick a different tswapAddres
...
//OK
    ...
    function deposit(IERC20 token, uint256 amount) external revertIfZero(amount) revertIfNotAllowedToken(token) {
        ...
        //MATHS ->
        /**
         100e18 USDC * 1e18 / 1e18 (2e18)
         = 50e18 FLASHLOAN TOKENS 
         */
         // e it should be never be 0
        uint256 mintAmount = (amount * assetToken.EXCHANGE_RATE_PRECISION()) / exchangeRate;
        emit Deposit(msg.sender, token, amount);
        assetToken.mint(msg.sender, mintAmount);
        
        // @ audit HIGH - CHANGING THE FEE 
        // qanswered why are we calculating the fees of the flash loans in the deposit?
        uint256 calculatedFee = getCalculatedFee(token, amount);
        // qanswered why are we updating the exchange rate?
        // wrong
...
//OK
    ...
    // I have 10 assetToken for USDC, let me get my USDC based on the 10 assetToken exchange rate
    function redeem(
        IERC20 token,
...
//OK
    ...
    function flashloan(
        address receiverAddress, // e The addres to get the loan
        ...
        revertIfNotAllowedToken(token)
    {
        ...
        // the fee of the flash loan - where the liquidy prov get profit
        uint256 fee = getCalculatedFee(token, amount);
        // slither-disable-next-line reentrancy-vulnerabilities-2 reentrancy-vulnerabilities-3
        // @follow-up - why it have to be greater
    ...
    //@ audit INFO Functions not used in this contract: Put external  
    //@ audit the parameter is address not IERC20
    ...
    // @param amount being borrowed
    // @param token the ERC20 being borrowed
    function getCalculatedFee(IERC20 token, uint256 amount) public view returns (uint256 fee) {
        //slither-disable-next-line divide-before-multiply
        
        // e this is why we need TSWAP -> getPriceInWeth
        // q is this correct?

        // 1 USDC
        // 0.1 WETH
        // 1e18
        // Why are we getting the fee in weth

        // 1 USDC == 0.1 WETH 
        // 1 USDC + 0.003 WETH
        // 1 USDC + 0.003 USDC 
        // @ audit HIGH if the fee is going to be in token, then the value should reflect that
        // IMPACT prices are wrong -> med/High |LIKELIHOOD -> HIGH
    ...
    function updateFlashLoanFee(uint256 newFee) external onlyOwner {
        ...
        // @ audit [low] should emit an event to notify
    ...
    //@ audit [INFO] Writen in aderyn - Functions not used in this contract: Put external  
